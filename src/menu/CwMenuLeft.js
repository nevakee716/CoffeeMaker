...
(function (cwApi) {
  ...
  cwApi.cwMenuLeft = (function () {
    ...
    function outputMenuZone(o, type, href, viewName, title, icon, badgeNum, badgeNumMaxExceeded) {
      ...
      } else if (viewName === "separatortext") {
        o.push('<a class="separatorTextMenu" href="#/homepage=true">');
      } else {
    ...
    function outputMenu(links, o) {
      ...
      var checkIfRole = function (menuConfig, menu) {
        ...
        if (menuConfig.dontRoles && Object.keys(menuConfig.dontRoles).length > 0) {
          ...
          for (var i = 0; i < currentUser.RolesId.length; i++) {
            if (menuConfig.dontRoles.hasOwnProperty(currentUser.RolesId[i])) return false;
          }
        }
        if (menuConfig.roles && Object.keys(menuConfig.roles).length > 0) {
          var currentUser = cwApi.currentUser;
          for (var i = 0; i < currentUser.RolesId.length; i++) {
            if (menuConfig.roles.hasOwnProperty(currentUser.RolesId[i])) return true;
          }
          return false;
        }
        return true;
      ...
      for (i = 0; i < links.length; i += 1) {
        menu = links[i];
        if (menu && checkIfRole(config?.menus?.[menu.title], menu) && (menu.Children.length > 0 || menu.type === "index" || menu.href !== "#")) {
